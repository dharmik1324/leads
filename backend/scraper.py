...
def scrape_maps_leads(query, max_results=20, log_callback=None, stop_check_callback=None):
    """
    ...
    with sync_playwright() as p:
        ...
        while len(leads) < max_results:
            if stop_check_callback and stop_check_callback():
                log_callback("Stop signal detected. Aborting search...")
            ...
            for card_url, unique_key in new_urls:
                if stop_check_callback and stop_check_callback():
                    log_callback("Stop signal detected. Aborting item processing...")
                    raise InterruptedError("Stopped by user")
